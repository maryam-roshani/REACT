...
const ExampleInput = (props) => {
    return (
      <input type= {props.type} value = {props.value} className="p-3 shadow-md rounded-md outline-0 w-full" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter your " + {props.name}/>
    )
}
          ...
          <div className="flex justify-content-center bg-white rounded-lg w-1/3 m-auto">
            ...
            <h1 className='mb-3 mx-1 text-xl font-semibold '>Login</h1>
              <form className='grid gap-3'>
                ...
                <button className='border-none p-3 text-lg mb-2 bg-red-500 rounded-md font-semibold text-white' onClick={()>Continue</button>
                    <div className="">No account? <span className='font-bold cursor-pointer text-red-600' onClick={()=>{setAction("Sign Up")}}>Sign Up</span>
                    </div>
              </form>
            </div>
          </div>
